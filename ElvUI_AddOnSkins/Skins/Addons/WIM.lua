...
local WIMReadyCheckCancelID = nil

-- WIM 3.3.7

function E:RegisterSkinForWIM(WIM_Elvui)
	if WIM and WIM.db then
		WIM.db.skin.selected = "WIM ElvUI"
		WIM.RegisterSkin(WIM_Elvui)
		if WIMReadyCheckCancelID then
			E:CancelTimer(WIMReadyCheckCancelID)
		end
	end
end
...
local function LoadSkin()
	...
	if IsAddOnLoaded("WIM") then
		WIMReadyCheckCancelID = E:ScheduleRepeatingTimer("RegisterSkinForWIM", 2, WIM_Elvui)
	end
